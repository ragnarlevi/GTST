...
parser.add_argument('-k', '--AverageDegree', type=float,metavar='', help='Average degree of each graph. Shared in both samples.')
parser.add_argument('-exp', '--exponent', type=float,metavar='', help='exponent of the scale free distribution')
parser.add_argument('-b1', '--balance1', type=float,metavar='', help='Balance ratio of the graphs in sample 1')
parser.add_argument('-b2', '--balance2', type=float,metavar='', help='Balance ratio of the graphs in sample 2')
...
if __name__ == "__main__":
    ...
    bg1 = mg.SignedGraph(n1, nnode1, k = avg_degree, exponent = exp, l = 'edges', fullyConnected = True, balance_target = b1)
    bg2 = mg.SignedGraph(n2, nnode2, k = avg_degree, exponent = exp, l = 'edges', fullyConnected = True, balance_target = b2)
    ...
    # only for random walk kernel
    edge_labels = [-1, 1]

    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = [executor.submit(mg.iteration, n , kernel, normalize, MMD_functions, bg1, bg2, B, kernel_hypothesis, kernel_library, edge_labels_tag = 'sign', edge_labels = edge_labels) for n in [part] * d]
    ...
    for alpha in alphas:
        ...
        tmp = pd.DataFrame({'kernel': str(kernel), 
                        ...
                        'nr_nodes_1': nnode1,
                        'nr_nodes_2': nnode2,
                        'p_edge_1': avg_degree/float(nnode1-1),
                        'p_edge_1': avg_degree/float(nnode2-1),
                        'degree': avg_degree,
                        'balance_ration_1':b1,
                        'balance_ration_2':b2,
                        'balance_difference': b1 - b2,
                        'n':n1,
                        'm':n2,
                        'timestap':time,
                        'B':B,

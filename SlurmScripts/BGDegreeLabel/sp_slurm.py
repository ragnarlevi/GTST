...
for nr_node in nr_nodes:
    for nr_sample in nr_samples:
        for k_off in degree_offsets:
            ...
            f"#SBATCH --job-name=sp_mmd_{tt}",
            f"#SBATCH --partition=amd-longq",
            f"#SBATCH --nodes=1",
            f"#SBATCH --ntasks-per-node=1",
            f"#SBATCH --cpus-per-task={cpu_per_task}",
            f"#SBATCH --output=/home/rgudmundarson/projects/MMDGraph/outputs/name=mmd_experiment_sp_v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.out",
            f"#SBATCH --error=/home/rgudmundarson/projects/MMDGraph/errors/name=mmd_experiment_sp_v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.err",

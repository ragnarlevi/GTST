...
for nr_node in nr_nodes:
    for nr_sample in nr_samples:
        for k_off in degree_offsets:
            ...
            f"#SBATCH --job-name=graphstat_degreelabel_{nr_node}_n_{nr_sample}_k_{k_off}",
            f"#SBATCH --partition=amd-longq",
            f"#SBATCH --nodes=1",
            f"#SBATCH --ntasks-per-node=1",
            f"#SBATCH --cpus-per-task={cpu_per_task}",
            f"#SBATCH --output=/home/{usr}/projects/MMDGraph/outputs/graphstat_v_{nr_node}_n_{nr_sample}_k_{k_off}.out",
            f"#SBATCH --error=/home/{usr}/projects/MMDGraph/errors/graphstat_v_{nr_node}_n_{nr_sample}_k_{k_off}.err",
            ...
            items.append(f"python3 Experiments/BGDegreeLabel/graphstats.py -B {B} -N {N} -p {data_name}  -n1 {nr_sample} -n2 {nr_sample} -nnode1 {nr_node} -nnode2 {nr_node} -k1 {k} -k2 {k + k_off} -d {cpu_per_task}")

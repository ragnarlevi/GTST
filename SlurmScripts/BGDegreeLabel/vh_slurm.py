...
for nr_node in nr_nodes:
    for nr_sample in nr_samples:
        for k_off in degree_offsets:
            ...
            f"#SBATCH --job-name=vh_bgdeg_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}",
            ...
            items.append(f"python3 Experiments/BGDegreeLabel/vh.py -B {B} -N {B} -p {data_name} -norm {norm} -n1 {nr_sample} -n2 {nr_sample} -nnode1 {nr_node} -nnode2 {nr_node} -k1 {k} -k2 {k + k_off} -d {cpu_per_task}")

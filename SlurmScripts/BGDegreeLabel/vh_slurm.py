...
for nr_node in nr_nodes:
    for nr_sample in nr_samples:
        for k_off in degree_offsets:
            ...
            if tt.lower() == "bgdegreelabel":
                data_name = f'data/BGDegreeLabel/VH/v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.pkl'
            elif tt.lower() == "bgsamelabel":
                data_name = f'data/BGSameLabel/VH/v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.pkl'
            
            job_file = path + f"/VH/v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.slurm"

            items = ["#!/bin/bash", 
            f"#SBATCH --time=3:00:00",
            f"#SBATCH --job-name=vh_{tt}_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}",

...
for nr_node in nr_nodes:
    for nr_sample in nr_samples:
        for k_off in degree_offsets:
            ...
            if tt.lower() == "bgdegreelabel":
                data_name = f'data/BGDegreeLabel/VH/wl_v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.pkl'
            elif tt.lower() == "bgsamelabel":
                data_name = f'data/BGSameLabel/VH/wl_v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.pkl'
            
            job_file = path + f"/VH/v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.slurm"

            items = ["#!/bin/bash", 
            f"#SBATCH --time=3:00:00",
            f"#SBATCH --job-name=wloa_{tt}_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}",
            f"#SBATCH --partition=amd-longq",
            f"#SBATCH --nodes=1",
            f"#SBATCH --ntasks-per-node=1",
            f"#SBATCH --cpus-per-task={cpu_per_task}",
            f"#SBATCH --output=/home/rgudmundarson/projects/MMDGraph/outputs/vh_v{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.out",
            f"#SBATCH --error=/home/rgudmundarson/projects/MMDGraph/errors/vh_v_{nr_node}_n_{nr_sample}_k_{k_off}_norm_{norm}.err",
            ...
            if tt.lower() == "bgdegreelabel":
                ...
                items.append(f"python3 Experiments/BGSameLabel/vh.py -B 1000 -N 1000 -p {data_name} -s 1 -norm {norm} -n1 {nr_sample} -n2 {nr_sample} -nnode1 {nr_node} -nnode2 {nr_node} -k1 {k} -k2 {k + k_off} -d {cpu_per_task}")

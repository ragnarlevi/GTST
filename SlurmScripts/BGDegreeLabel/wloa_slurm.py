    ...
    for nr_sample in nr_samples:
        for k_off in degree_offsets:
            ...
            if tt.lower() == "bgdegreelabel":
                ...
                data_name = f'data/BGSameLabel/WLOA/wl_v_{nr_node}_n_{nr_sample}_k_{k_off}_wl_{wl_it}_norm_{norm}.pkl'
            ...
            if tt.lower() == "bgdegreelabel":
                items.append(f"python3 Experiments/BGDegreeLabel/wloa.py -B 1000 -N 1000 -p {data_name} -s 1 -norm {norm} -nitr {wl_it} -n1 {nr_sample} -n2 {nr_sample} -nnode1 {nr_node} -nnode2 {nr_node} -k1 {k} -k2 {k + k_off} -d {cpu_per_task}")
            elif tt.lower() == "bgsamelabel":
                items.append(f"python3 Experiments/BGSameLabel/wloa.py -B 1000 -N 1000 -p {data_name} -s 1 -norm {norm} -nitr {wl_it} -n1 {nr_sample} -n2 {nr_sample} -nnode1 {nr_node} -nnode2 {nr_node} -k1 {k} -k2 {k + k_off} -d {cpu_per_task}")

...
if kernel_name == 'gh':
    ...
    k_val = 'HASH'
    unique_identifier = f'its_{ksp["iterations"]}_scale_{ksp["scale"]}_w_{ksp["w"]}_nitr_{ksp["nitr"]}_bkernel_{ksp["basekernel"]}_sigma_{ksp["sigma"]}'
    script_args = f'-kernel {kernel_name} -iterations {ksp["iterations"]} -w {ksp["w"]} -scale {ksp["scale"]} -nitr {ksp["nitr"]} -basekernel {ksp["basekernel"]} -sigma {ksp["sigma"]}'
...
Path(f"data/SBMAttributeNormal/{k_val}").mkdir(parents=True, exist_ok=True)
Path(f"/home/{usr}/projects/MMDGraph/SlurmBatch/SBMAttributeNormal/{k_val}").mkdir(parents=True, exist_ok=True)
...
for nr_sample in nr_samples:
    for nnode in nnodes:
        for mdiff in mean_diffs:
            ...
            data_name = f'data/SBMAttributeNormal/{k_val}/n_{nr_sample}_nnode{nnode}_mdiff_{mdiff}_norm_{norm}_{unique_identifier}.pkl'
            ...
            f"#SBATCH --output=/home/{usr}/projects/MMDGraph/outputs/{k_val}_sbmorl_n_{nr_sample}_nnode{nnode}_mdiff_{mdiff}_norm_{norm}_{unique_identifier}.out",
            f"#SBATCH --error=/home/{usr}/projects/MMDGraph/errors/{k_val}_sbmorl_n_{nr_sample}_nnode{nnode}_mdiff_{mdiff}_norm_{norm}_{unique_identifier}.err",
            ...
            items.append(f"python3 Experiments/SBMAttributeNormal/run.py -B {B} -N {N} -n1 {nr_sample} -n2 {nr_sample} -nnode1 {nnode} -nnode2 {nnode} -mean11 {mean11} -mean12 {mean12} -mean13 {mean13} -mean21 {mean21} -mean22 {mean22} -mean23 {mean23} -p {data_name} -norm {norm} -d {cpu_per_task} -{script_args}")

...
CODE FROM https://github.com/srivastavaprashant/mgarch
...
- ((dof+ self.N)*( ((np.matmul(self.rt[i], (np.matmul( np.linalg.inv(H_t[i]), self.rt[i].T))))/(dof - 2.)) + 1)/2.)
    ...
    def fit(self, returns):
        ...
        self.D_t = D_t
        print("garch finished, onto DCC")
        if self.dist == 'norm':
            res = minimize(self.mgarch_loglike, np.array([0.01, 0.94]), args = D_t,

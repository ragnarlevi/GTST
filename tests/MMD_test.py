...
def test_MMD():
    ...
    MMD_out = MMD()
    MMD_out.fit(G1 = g1, G2 = g2, kernel = 'RW_ARKU_plus', mmd_estimators = ['MMD_u', 'MMD_b', 'MMD_l'], r = 6, c = 1e-4, verbose = True, node_attr = 'attr',
                bootstrap_method = 'BootstrapScheme', check_symmetry = True)
    assert_low_p_val(MMD_out.p_values)

    g1, g2 = generate_H0_false_but_same_topology()
    MMD_out = MMD()
    MMD_out.fit(G1 = g1, G2 = g2, kernel = 'RW_ARKU_edge', mmd_estimators = ['MMD_u', 'MMD_b', 'MMD_l'], r = 6, c = 1e-4, verbose = True, 
                edge_label = 'edge_label', unique_edge_labels = ['a', 'b'], check_symmetry = True)
    assert_low_p_val(MMD_out.p_values)

    g1, g2 = generate_H0_false_but_same_topology()
    MMD_out = MMD()
    MMD_out.fit(G1 = g1, G2 = g2, kernel = 'RW_ARKL', mmd_estimators = ['MMD_u', 'MMD_b', 'MMD_l'], r = 6, c = 1e-4, verbose = True, 
                node_label = 'label', unique_node_labels = ['a', 'b', 'c'])
    assert_low_p_val(MMD_out.p_values)

    g1, g2 = generate_H0_false_directed()
    MMD_out = MMD()
    MMD_out.fit(G1 = g1, G2 = g2, kernel = 'RW_ARKU', mmd_estimators = ['MMD_u', 'MMD_b', 'MMD_l'], r = 6, c = 1e-4, verbose = True, edge_attr = 'weight')
    assert_low_p_val(MMD_out.p_values)

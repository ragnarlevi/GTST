...
def test_RandomWalk_ARKU_plus():
    ...
    rw = RandomWalk(g1 + g2,r = 3, c = 1e-5)
    ...
    rw = RandomWalk(g1 + g2,r = 3, c = 1e-5,edge_attr='weight')
    rw.fit('ARKU_plus', verbose=True)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2)

    rw = RandomWalk(g1 + g2,r = 3, c = 1e-8,node_attr='attr')
    rw.fit('ARKU_plus', verbose=True)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2)
...
def test_RandomWalk_ARKL():
    ...
    rw = RandomWalk(g1 + g2, r = 3, c = 0.001, node_label='label', unique_node_labels = unique_node_labels)
...
def test_RandomWalk_ARKU():
    g1, g2 = generate_H0_false()
    rw = RandomWalk(g1 + g2, r = 3, c = 0.001)
...
def test_RandomWalk_ARKU_edge():
    g1, g2 = generate_H0_false_but_same_topology()
    rw = RandomWalk(g1 + g2, r = 3, c = 0.0001, edge_label='edge_label',unique_edge_labels= ['a', 'b'])

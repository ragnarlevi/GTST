...
def test_RandomWalk_ARKU_plus():
    ...
    rw = RandomWalk(g1 + g2,r = 6, c = 1e-3)
    ...
    assert np.all(np.fromiter(pval.p_values.values(), dtype=float) < 0.01), f"Some p value too high {pval.p_values} for RW binary"

    
    g1, g2 = generate_H0_false_but_same_topology()
    rw = RandomWalk(g1 + g2,r = 6, c = 1e-3,edge_attr='weight')
    rw.fit('ARKU_plus', verbose=False)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKU weight")

    rw = RandomWalk(g1 + g2,r = 6, c = 1e-3,node_attr='attr', normalize=True)
    rw.fit('ARKU_plus', verbose=True)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKU attr")
...
def test_RandomWalk_ARKL():
    ...
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKL diff topology")

    g1, g2 = generate_H0_false_but_same_topology()
    unique_node_labels= ['a', 'b', 'c']
    rw = RandomWalk(g1 + g2, r = 6, c = 0.001, node_label='label', unique_node_labels = unique_node_labels)
    rw.fit('ARKL', verbose=False)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKL same topology")


def test_RandomWalk_ARKU():
    g1, g2 = generate_H0_false()
    rw = RandomWalk(g1 + g2, r = 6, c = 0.001)
    rw.fit('ARKU',verbose=False)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKU same topology")
    # Directed
    g1, g2 =  generate_H0_false_directed()
    rw = RandomWalk(g1 + g2, r = 6, c = 0.001)
    rw.fit('ARKU',verbose=False)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKU directed")


def test_RandomWalk_ARKU_edge():
    g1, g2 = generate_H0_false_but_same_topology()
    rw = RandomWalk(g1 + g2, r = 6, c = 0.0001, edge_label='edge_label',unique_edge_labels= ['a', 'b'])
    rw.fit('ARKU_edge', verbose=False)
    assert_positive_eig(rw.K)
    assert_low_p_val_from_K(rw.K, g1,g2, "RW ARKU_edge")
...
def test_GNTK():
    ...
    gntk_kernel = GNTK(num_layers= 2, num_mlp_layers = 2, 
                                jk = True, scale = 'uniform', normalize=0)
    gntk_kernel.fit_all(g1 + g2,degree_as_tag=True,features= None)
    assert_positive_eig(gntk_kernel.K)
    assert_low_p_val_from_K(gntk_kernel.K, g1,g2, "GNTK diff topology")

    gntk_kernel = GNTK(num_layers= 2, num_mlp_layers = 2, 
                            jk = False, scale = 'uniform', normalize=1)
    gntk_kernel.fit_all(g1 + g2,degree_as_tag=True,features= None)
    assert_positive_eig(gntk_kernel.K)
    assert_low_p_val_from_K(gntk_kernel.K, g1,g2, "GNTK same topology")
    ...
    assert_positive_eig(gntk_kernel.K)
    assert_low_p_val_from_K(gntk_kernel.K, g1,g2, "GNTK same topology attr")
...
def test_DK():
    ...
    assert_low_p_val_from_K(dk_kernel.K, g1,g2, "DK same topology wl")


    dk_kernel = DK(type = 'sp', opt_type = None,
                            vector_size= 2, window=2, min_count=0, 
                            normalize = 1, nodel_label='label')
    dk_kernel.fit_transform(g1+g2)
    assert_positive_eig(dk_kernel.K)
    assert_low_p_val_from_K(dk_kernel.K, g1,g2, "DK same topology sp")
    ...
    dk_kernel.fit_transform(g1+g2)
    assert_positive_eig(dk_kernel.K)
    assert_low_p_val_from_K(dk_kernel.K, g1,g2, "DK same topology word2vec")


def test_WWL():
    g1, g2 = generate_H0_false_but_same_topology()

    wwl_kernel = WWL(param =dict(discount=0.1, h=2, sinkhorn=False, label_name='label', normalize = True))
    wwl_kernel.fit_transform(g1+g2)
    assert_positive_eig(wwl_kernel.K)
    assert_low_p_val_from_K(wwl_kernel.K, g1,g2, "WWL same topology")

...
EBIC_tlasso <- function(wi, x, tau, N, rho_matrix, beta = 0.5){
...
fit_one_sample <- function(x){
  ...
  for(rho in seq(from = 0, to = 0.5, by = 0.02)){
    ...
    info[[paste0(rho, "normal")]] <- data.frame(ebic = EBIC_out$EBIC, loglik = EBIC_out$loglik, BIC = EBIC_out$BIC, AIC = EBIC_out$AIC, FDR = num_edges_false/num_edges, FNR = num_missing_edges_f/num_missing_edges, model = "normal", rho = rho)
    ...
    info[[paste0(rho, "Tyler")]] <- data.frame(ebic = EBIC_out$EBIC, loglik = EBIC_out$loglik, BIC = EBIC_out$BIC, AIC = EBIC_out$AIC, FDR = num_edges_false/num_edges, FNR = num_missing_edges_f/num_missing_edges, model = "Tyler", rho = rho)
    ...
    EBIC_out <- EBIC_tlasso(precision_tlasso, out_tlasso$S_tau, N, rho_matrix = rho_matrix)
    info[[paste0(rho, "tlasso")]] <- data.frame(ebic = EBIC_out$EBIC, loglik = EBIC_out$loglik, BIC = EBIC_out$BIC, AIC = EBIC_out$AIC, FDR = num_edges_false/num_edges, FNR = num_missing_edges_f/num_missing_edges, model = "tlasso", rho = rho)
...
```{r}
sector_tmp <- list("Basic Materials" = sector_classification[["Basic Materials"]])
...
```{r}
ggplot(out_returns_adaptive_bm$melted_cormat %>% filter(time == "2015-03-26", id != "Heavy_Spectral")) + geom_line(aes(x = rho, y = abs(val), group = paste(col, row))) + facet_wrap(~id)


df <- out_returns_adaptive_bm$information_criteria

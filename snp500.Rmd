...
EBIC <- function(wi, Csample, N, beta = 0.5){
  ...
  #
        ...
        loglik <- 0.5*N*sum(log(w)) - 0.5*N*sum(diag(wi %*% Csample)) - 0.5*p*N*log(2*pi)
        
        # get upper triangle
        ind <- which( upper.tri(wi,diag=FALSE) , arr.ind = TRUE )
...
EBIC_spectral_heavy <- function(L, data, N, nu, beta = 0.5){
  ...
  log_sum <- sum(apply(data_log_retuns, 1, function(x){log(1 + x %*% corr_out %*% x)}))
  
  loglik <- const*log_sum -  - sum(log(w))
...
Select only companies with ESG ratings.

```{r}
log_returns_median_regression_esg <- log_returns_median_regresion[, names(log_returns_median_regresion) %in% names(ESG_y_smooth)]
log_returns_esg <- log_returns[, names(log_returns) %in% names(ESG_y_smooth)]
```
...
fit_glasso_models <- function(corr_out, rho, n_samples, id, sector, time, adaptive_lasso = TRUE){
  ...
  # EBIC
  EBIC_out <- EBIC(-neg_prec, corr_out, n_samples)
...
fit_tlasso_models <- function(data, mle, nu, rho, n_samples, id, sector, time, adaptive_lasso = TRUE){
  ...
  if(adaptive_lasso){
    ...
    rho_matrix <- rho*matrix(rep(1, nrow(corr_out)^2), nrow = ncol(data))
  }
  ...
  # EBIC
  EBIC_out <- EBIC(-neg_prec, out_tlasso$S_tau, n_samples)
  ebic <- data.frame(ebic = EBIC_out$EBIC, loglik = EBIC_out$loglik, BIC = EBIC_out$BIC, AIC = EBIC_out$AIC, id = id)
  ebic$rho <- rho
  ebic$sector <- sector
  ebic$time <- time
  ...
  diag(neg_prec) <- 0
  rownames(neg_prec) <- names(data)
  colnames(neg_prec) <- names(data) 
  ind <- which( upper.tri(neg_prec,diag=TRUE) , arr.ind = TRUE )
  melted_cormat <- data.frame( col = colnames(neg_prec)[ind[,2]] ,
                                  row = rownames(neg_prec)[ind[,1]] ,
                                  val = neg_prec[ ind ] )
...
fit_spectral_model <- function(data, nu, time, n_samples, sector, id){
  ...
  rownames(A_spectral_heavy) <- names(data) 
  colnames(A_spectral_heavy) <- names(data) 
  ...
  return(list(A = A, information = ebic, melted_cormat = melted_cormat))  
}



fit_precisions <- function(data, data_no_market, window_size, sector_classification, dates, adaptive_lasso = TRUE, step_forward = 5){
  ...
  rhos <- c(seq(0.001, 0.15, 0.004), 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.6, 0.8, 1.5, 2, 5, 8, 100)
  ...
  for(sector in names(sector_classification)){
    ...
    for(t in seq(from = 1, to = nrow(data)-window_size, by = step_forward)){
      ...
      nu_no_market <- fit_mvt(data_no_market,nu="MLE-diag-resample")$nu  # for heavy distributions
      nu <- fit_mvt(data,nu="MLE-diag-resample")$nu
      corr_out_normal <- cor(data_no_market_t)
      out_tyler <- fit_Tyler(data_no_market_t)
      ...
      for(rho in rhos){
        ...
        # Normal
        out_normal <- fit_glasso_models(corr_out_normal, rho, window_size, "normal", sector, time, adaptive_lasso)
        ...
        # Tyler
        out_tyler <- fit_glasso_models(corr_out_tyler, rho, window_size, "tyler", sector, time, adaptive_lasso)
        ...
        # Tlasso
        out_tlasso <- fit_tlasso_models(data_no_market_t,precision_mle_tlasso, nu_no_market, rho, window_size, "Tlasso", sector, time, adaptive_lasso)
      ...
      # Spectral Heavy
      # out_spectral_heavy <- fit_spectral_model(data_t, nu, time, window_size, sector, "Heavy_Spectral")
...
```{r}
dates <- rownames(log_returns_esg)

out_returns_adaptive <- fit_precisions(log_returns_esg, log_returns_median_regression_esg, 125, sector_classification, dates,  TRUE,  5)
save.image()
```

...
# # SP
# print('sp')
# os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/SP/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_norm_0_wlab_{0}_k_{0.25}.pkl -B 10000 -N 6000 -kernel sp -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -wlab 0")
# print('sp')
# os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/SP/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_norm_0_wlab_{1}_k_{0.25}.pkl -B 10000 -N 6000 -kernel sp -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -wlab 1")

# VH
print('vh')
os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/VH/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_norm_0_k_{0.25}.pkl -B 10000 -N 6000 -kernel vh -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 ")
...
for l in [0.1, 0.001, 1]:
    for nitr in [1, 2,4, 6, 8]:
        print(f'{l} {nitr}')
        os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/WWL/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_nitr_{nitr}_l_{l}_norm_0_k_{0.25}.pkl -B 10000 -N 6000 -kernel wwl -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -nitr {nitr} -l {l}")

# # PROP
for tmax in [1,2,3,4,5]:
    for w in [0.01, 0.001, 0.0001]:
        print(f'{tmax} {w}')
        os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/PROP/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_tmax_{tmax}_M_TV_w_{w}_norm_0_diff_0.1.pkl -B 1000 -N 6000 -kernel prop -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -tmax {tmax} -w {w} -M H")
...
for L in [8, 10, 12]:
    for d in [2]:
        print(f'{L} {d}')
        os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/PYRAMID/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_L_{L}_dim_{d}_norm_0_diff_0.1.pkl -B 10000 -N 6000 -kernel pyramid -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -L {L} -dim {d} -wlab 0")

# DK
print('dk')
os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/DK/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_type_sp_norm_0_diff_0.1.pkl -B 10000 -N 6000 -kernel dk -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -type sp")
for nitr in [1, 2, 4, 6]:
    print(f'{nitr}')
    os.system(f"python Experiments/ScaleFree/run.py -p data/ScaleFree/DK/n1_{n1}_n2_{n2}_v1_{v1}_v2_{v2}_type_sp_nitr_{nitr}_norm_0_diff_0.1.pkl -B 10000 -N 6000 -kernel dk -nnode1 {v1} -nnode2 {v2} -k1 {k1} -k2 {k2} -n1 {n1} -n2 {n2} -d 4 -norm 0 -type wl -nitr {nitr}")

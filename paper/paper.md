...
In the two-sample testing of graph-valued, we assume we are given two sets of samples/observations that comprise collections of graph-valued data $\{G_1,...,G_{n}\}$ and $\{G'_1,...,G'_{n'}\}$ where $G_i, G'_j \in \Omega, \quad \forall i,j$. The graphs in the two samples are all generated independently from two probability spaces $(\Omega, \cF, \mathbbm{P})$  and $(\Omega, \cF, \mathbbm{Q})$, and the goal is to infer whether $ \mathbbm{P} = \mathbbm{Q}$. 

It is worth pausing for a moment to inspect the probability spaces more closely. In the simplest case, the sample space $\Omega$ contains all possible edges that can occur in a graph $G$, that is $\Omega = \{ (v_1, v_2), \dots, (v_1, v_{|V|}), (v_2, v_1), \dots, (v_{|V|}, v_{|V|-1}) \} $ \footnote{We are assuming that a node can not be connected to itself}. As the sample space is discrete we can define the $\sigma$-algebra as the power set of $\Omega$, namely, $\mathcal{F} = \mathcal{P}(\Omega)$. The probability function $\mathbbm{P}: \mathcal{F} \mapsto [0,1]$ then defines the probability of obtaining a certain graph in the sample set of graph-valued data. As an example we can define for instance a population distribution to be uniform  $\mathbbm{P}(G(|V|,|E|)) = \frac{1}{{M \choose |E|}}$ where $M = {|V| \choose 2}$ is the total number of possible edges and $G(|V|,|E|)$ is a graph with $|V|$ vertices and $|E|$ number of edges.  This setting is illustrated in figure \ref{two_sample_testing}


  ![The Graph two sample testing scenario. Here we have observed 4 graphs from $\mathbbm{P}$ and 3 graphs from $\mathbbm{Q}$. The sample space of $\mathbbm{P}$ and $\mathbbm{Q}$ is the same \big($5 \choose 2$ possible edges\big) \label{two_sample_testing}](two_sample_testing.PNG)
...
\label{eq:hyp_test}
\begin{split}
    H_0:\quad & \mathbbm{P} = \mathbbm{Q} \\
    H_1:\quad & \mathbbm{P} \neq \mathbbm{Q}.
\end{split}
\end{equation}


The test statistic used in this case is the largest distance between expectation of some function w.r.t. to the two probability distributions. Let $\mathcal{H}_B$ be a class of functions $f: \Omega \to \mathbbm{R}$. The maximum mean discrepancy (MMD) is defined as:
\begin{equation}
    \label{eq:mmd}
    \textrm{MMD}[\cH_B,\mathbb{P}, \mathbbm{Q}] := \sup_{f \in \cH_B} \big( \E_{G \sim\mathbbm{P}}[f(G)] - \E_{G' \sim \mathbbm{Q}}[f(G')] \big).
\end{equation}

When the class of function, $\mathcal{H}_B$, is the unit ball in a RKHS, then the squared population MMD becomes:

\begin{equation}
\label{eq:mmd_expecation}
\begin{split}
   \textrm{MMD}^2[\mathcal{H}_1, {\mathbbm{P}}, {\mathbbm{Q}}] &=\E_{\mathbbm{P}, \mathbbm{P}}[k(G,G)] - 2\E_{\mathbbm{P}, \mathbbm{Q}}[k(G,G')] + \E_{\mathbbm{Q}, \mathbbm{Q}}[k(G',G')],
    \end{split}
\end{equation}

where $k$ is some graph kernel. Finally unbised, biased, and robust estimators of $ \textrm{MMD}^2[\mathcal{H}_1, {\mathbbm{P}}, {\mathbbm{Q}}]$ along with a permutation sampling are used to estimate a p-value for the statistical test [@Gretton2012,@MONK].
